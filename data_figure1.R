...
nyc_mortality_from_1916 = mutate(nyc_mortality_from_1916, threeweek_avg_flu= rollmean(total, 3, align="center", fill=0))

nyc_mortality_from_1916$threeweek_avg_flu[1] <- 2191
nyc_mortality_from_1916$threeweek_avg_flu[418] <- 1487.333

plot_flu <-ggplot(data=nyc_mortality_from_1916)+
  theme_bw()+ geom_col(aes(x=Date_date, y=total), fill="pink") +
  geom_line(aes(x=Date_date, y = threeweek_avg_flu), color = "red", size = .75) +
  ggtitle("Total Mortality in New York City, 1916-1924") + 
  xlab("Date") + ylab("Deaths") + 
  theme(plot.title = element_text(hjust = 0.5, size = 11)) + 
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank()) +
  scale_x_date(breaks = as.Date(c("1917-01-06", "1923-12-29")), date_breaks = "1 year", date_labels = "%Y")
plot_flu
...
g1 <- ggplotGrob(plot_flu)
g2 <- ggplotGrob(plot_h1n1_2009)  
g3 <- ggplotGrob(plot_cov_2020)
...
g <- set_panel_widths(g, lapply(1:3, grid::unit, "null")) 
grid::grid.draw(g)
