...
plot_h1n1_2009 <- ggplot(data=dat_from_2009)+
  ...
  ylab("Confirmed H1N1/Total Specimens * 100 (% Positive)") + xlab("Date") + 
...
plot_cov_2020 <-ggplot(data=covid_usa_streamlined)+
  ...
  theme(plot.title = element_text(hjust = 0.5, size = 11)) +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank()) +
  scale_y_continuous(name = "New Cases/Total Specimens * 100 (% Positive)", sec.axis = sec_axis(~.*coeff, name="New Cases of SARS-CoV-2")) 
plot_cov_2020
