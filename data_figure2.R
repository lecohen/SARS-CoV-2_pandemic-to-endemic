...
#download data from lecohen/SARS-CoV-2_pandemic-to-endemic github#
...
pie_chart + blank_theme + theme(axis.text.x=element_blank()) + ggtitle("Age Distribution of Pandemic H1N1 Over Four US Flu Seasons (CDC)") + theme(plot.title = element_text(hjust = 0.75))
...
pie_chart2 + blank_theme + theme(axis.text.x=element_blank()) + ggtitle("Age Distribution of Indonesia Dengue Hemorrhagic Fever Incidence per 100,000 Population (Karyanti, 2014)") + theme(plot.title = element_text(size = 10.5))
...
pie_chart3 <- covid_cases_age_bp + coord_polar("y", start=0)
pie_chart3 + blank_theme + theme(axis.text.x=element_blank()) + ggtitle("Age Distribution of US COVID-19 Incidence per 100,000 Population (CDC)") + theme(plot.title = element_text(hjust = 0.75))
