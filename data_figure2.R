...
nyc_mortality <- read_excel("~/Desktop/nyc_mortality_age_and_season.xlsx")
...
nyc_mortality_bp <- ggplot(data=nyc_mortality, aes(x="", y=percentage, fill=age)) + geom_bar(width = 1, stat = "identity") + facet_grid(~season)
pie_chart <- nyc_mortality_bp + coord_polar("y", start=0)
pie_chart1 <- pie_chart2 + blank_theme + theme(axis.text.x=element_blank()) + ggtitle("Age Distribution of Total Mortality in NYC Pre and Post 1918 Influenza Pandemic") + theme(plot.title = element_text(hjust = 0.5))
...
pie_chart3 <- pie_chart2 + blank_theme + theme(axis.text.x=element_blank()) + ggtitle("Age Distribution of Pandemic H1N1 Over Four US Flu Seasons (CDC)") + theme(plot.title = element_text(hjust = 0.5))
...
pie_chart5 <- pie_chart4 + blank_theme + theme(axis.text.x=element_blank()) + ggtitle("Age Distribution of Indonesia Dengue Hemorrhagic Fever Incidence per 100,000 Population (Karyanti, 2014)") + theme(plot.title = element_text(hjust = 0.5))
...
pie_chart7 <- pie_chart6 + blank_theme + theme(axis.text.x=element_blank()) + ggtitle("Age Distribution of US COVID-19 Incidence per 100,000 Population (CDC)") + theme(plot.title = element_text(hjust = 0.5))

plot_grid(pie_chart1, pie_chart3, pie_chart5, pie_chart7, ncol=1, nrow=4, align="v")
